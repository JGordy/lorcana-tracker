...
describe("dbService (Server-Side)", () => {
  ...
  describe("getCollection", () => {
    ...
    it("should fetch documents from databases service for other collections", async () => {
      ...
      expect(mockDatabases.listDocuments).toHaveBeenCalledWith("test_db", "decks", []);
      expect(docs).toHaveLength(1);
    });

    it("should return empty array gracefully on 404 or 503 transient errors", async () => {
      mockDatabases.listDocuments.mockRejectedValueOnce({ code: 503, message: "timeout" });
  ...
  describe("updateInventory", () => {
    it("should delete document if quantity is 0 or less", async () => {
      ...
      const result = await dbService.updateInventory("user-1", "mickey-card", 0, false);
    ...
    it("should update document if quantity is greater than 0", async () => {
      ...
      const result = await dbService.updateInventory("user-1", "mickey-card", 3, false);
    ...
    it("should create document if not found on update", async () => {
      ...
      const result = await dbService.updateInventory("user-1", "mickey-card", 2, false);
