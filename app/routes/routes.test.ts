...
describe("Route Loaders & Actions", () => {
  describe("/verify Loader", () => {
    it("should return error when userId or secret query params are missing", async () => {
      ...
      const response = await verifyLoader({ request, params: {}, context: {} } as any);
    ...
    it("should call authService.verifyEmail when params are present", async () => {
      ...
      const request = new Request("http://localhost:5173/verify?userId=u123&secret=sec789");
      const response = await verifyLoader({ request, params: {}, context: {} } as any);
      ...
      expect(response).toEqual({
        success: true,
        message: "Your email address has been successfully verified! You now have full access.",
        user: null,
      });
    });
  });
  ...
  describe("/ (Home) Loader and Action", () => {
    it("should return user from session in home loader", async () => {
      ...
      const response = await homeLoader({ request, params: {}, context: {} } as any);
    ...
    it("should handle auth-login intent and set session cookie in home action", async () => {
      ...
      const actionResponse: any = await homeAction({ request, params: {}, context: {} } as any);
      expect(authService.login).toHaveBeenCalledWith({
        email: "test@user.com",
        password: "secret123",
      });
      expect(actionResponse.data || actionResponse).toMatchObject({ success: true });
    });
  });
});
